...
class Game():
    ...
    def house_turn(self):
        ...
        self.turn += 1
        print(f"house's turn: {self.turn}") # debugging, borrame
        if self.turn == 1:
            self.houses_hand.append(self.draw())
            self.houses_score += self.values
            self.values = 0
            print(self.houses_hand) # debugging, borrame
        elif self.turn == 2:
            self.houses_hand.append(self.draw())
            self.houses_score += self.values
            self.values = 0
    ...
    def player_turn(self):
        ...
        print("player's turn:") # debugging, borrame
        for i in range(2):
        ...
        not_score_enough = True
        while not_score_enough:
